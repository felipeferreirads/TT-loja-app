...
export function CashPage() {
  ...
  const load = () => {
    setLoading(true)
    Promise.all([fetchCashEntries(), fetchRecurringExpenses(), fetchLaunchedRecurringIdsThisMonth()])
      .then(([e, r, launched]) => {
        setEntries(e)
        setRecurring(r)
    ...
    <div>
      ...
      {!loading && !error && (
        <>
          ...
          {entries.length === 0 ? (
            <p className="text-sm text-stone-400">Nenhum lançamento registrado ainda.</p>
          ) : (
